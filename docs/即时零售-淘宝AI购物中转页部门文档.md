# 即时零售项目：金豆芽 × 千问 AI 购物中转页

## 1. 项目概述

本项目是面向即时零售投放场景的移动端 H5 中转页，用于承接二维码扫码流量，引导用户复制淘宝闪购口令，并尝试唤起千问 App 完成 AI 购物。

当前活动主体为“金豆芽 × 千问 AI 购物”。用户通过线下物料、渠道海报或其他投放入口扫码进入活动页后，页面会根据 URL 渠道参数匹配对应购物口令。用户点击按钮后，系统先复制口令，再打开千问 App；如果 App 未成功唤起，则显示对应系统的千问下载入口和重新打开按钮。

项目已配置为纯静态导出，可部署到静态托管、OSS、CDN、Nginx 静态目录等环境。

## 2. 业务目标

- 承接即时零售投放二维码流量，降低用户从扫码到千问 AI 购物的跳转成本。
...
5. 用户点击“复制口令并打开千问”。
6. 页面复制口令，并尝试唤起千问 App。
7. 如果千问 App 未成功唤起，显示对应系统的下载入口和重新打开按钮。
8. 微信内置浏览器中会显示“请用浏览器打开”指引，避免微信环境下跳转千问不稳定。
...
千问 Scheme、平台识别和下载地址集中维护在 `lib/qianwen.ts`。
...
1. 调用浏览器剪贴板能力复制购物口令。
2. 上报复制点击和复制结果。
3. Android 和 iOS 使用 `tongyi://page/h5?...` 尝试唤起千问。
4. 鸿蒙使用 `tongyi://?source_type=scheme` 打开千问首页。
5. 1.8 秒后页面仍处于可见状态时，显示对应系统的下载入口和重新打开按钮。

### 7.4 微信浏览器处理

微信内置浏览器对外部 App 跳转限制较多。页面会识别微信环境，并显示“请用浏览器打开”指引，避免用户在微信内反复点击但无法稳定打开千问。
...
| 事件码 | 说明 |
| --- | --- |
| `click_copy_and_open` | 点击“复制口令并打开千问” |
...
2. 将 `out/` 目录发布到静态托管环境。
3. 确认域名可访问 `/tb-ai/`。
4. 使用正式二维码扫码验证页面、复制口令、千问跳转和友盟数据。
...
- 微信内置浏览器对千问 App 跳转限制较多，当前策略是引导用户使用外部浏览器打开。
...
- 增加渠道维度的转化漏斗看板，包括访问、复制成功、打开千问、下载兜底等指标。
